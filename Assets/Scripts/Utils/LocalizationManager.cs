...
public class LocalizationManager
{
    ...
    void LoadLanguageForCards(string language)
    {
        CurrentLanguage = language;
        TextAsset localizedFile = Resources.Load<TextAsset>(
            string.Format("Languages/CardsLocalization/cards_{0}", language));
    ...
    {
        TextAsset localizedFile = Resources.Load<TextAsset>(
            "missions/" + GM.missionPath + "/languages/" + "text_"+ CurrentLanguage);

        if(localizedFile == null)
        {
            Debug.LogError("Where no language file");
            return;
        }
