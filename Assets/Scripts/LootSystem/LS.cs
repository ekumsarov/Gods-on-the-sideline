...
public class LS
{
    ...
    public static void AddItem(string id, LS.LootType type, string to = "Player")
    {
        ...
        else
        {
            ...
            GM.Player.Group.AddSkills(to, item.passive, "loot", id);
        }
    ...
    public static void UnequipItem(LootType type, string to = "Player")
    {
        if (LS.instance.equip[to][type] != null)
        {
            ...
            GM.Player.Group.RemoveSkills(to, LS.instance.equip[to][type].passive, "loot", LS.instance.equip[to][type].ID);
    ...
    public static void RemoveItem(string id)
    {
        ...
        foreach (var hero in LS.instance.equip)
        {
            if (hero.Value[LootType.Weapon].ID.Equals(id))
            {
                ...
                GM.Player.Group.RemoveSkills(hero.Key, hero.Value[LootType.Weapon].passive, "loot", hero.Value[LootType.Weapon].ID);
            ...
            if (hero.Value[LootType.SecondWeapon].ID.Equals(id))
            {
                ...
                GM.Player.Group.RemoveSkills(hero.Key, hero.Value[LootType.SecondWeapon].passive, "loot", hero.Value[LootType.Weapon].ID);
            ...
            if (hero.Value[LootType.Amulet].ID.Equals(id))
            {
                ...
                GM.Player.Group.RemoveSkills(hero.Key, hero.Value[LootType.Amulet].passive, "loot", hero.Value[LootType.Weapon].ID);
    ...
    {
        foreach(Hero hero in GM.Player.Group.GetUnits())
        {
            if(!LS.instance.equip.ContainsKey(hero.Name))
            {
                ...
                if (hero.EquippedLoot != null && hero.EquippedLoot.Count > 0)
                {
                    for(int i = 0; i < hero.EquippedLoot.Count; i++)
                    {
                        LootItem item = IOM.GetLootItem(hero.EquippedLoot[i]);
                        if(item == null)
                        {
                            Debug.LogError("No item ID: " + hero.EquippedLoot[i]);
                            continue;
                        }
                        ...
                        GM.Player.Group.AddSkills(hero.Name, item.passive, "loot", LS.instance.equip[hero.Name][item.type].ID);
                    }
                }
            }
        }
