...
public class EventPackManager
{
    ...
    void getAllAssets()
    {
        ...
        foreach (var a in asset)
            foreach (var t in a.GetExportedTypes())
                if (t.IsSubclassOf(typeof(EventPackSystem.EventPack)))
                {
                    ...
                    else if(typeName.IndexOf("EventPackSystem" + GM.missionPath + ".") == 0)
                    {
                        shortName = typeName.Substring(string.Concat("EventPackSystem.", GM.missionPath).Length);
                        Debug.Log(shortName);
                        assets[shortName] = t;
                    }
        ...
        TextAsset[] allTexts = Resources.LoadAll<TextAsset>("missions/" + GM.missionPath + "/EditorQuest");
