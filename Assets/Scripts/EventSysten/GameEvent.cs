...
namespace GameEvents
{
    ...
    public class GameEvent : ObjectID, ICloneable
    {
        ...
        {
            foreach(string fd in param.Keys)
            {
            ...
            foreach (var a in asset)
            {
                foreach (var t in a.GetExportedTypes())
                    if (t.IsSubclassOf(typeof(GameEvents.GameEvent)))
                    {
                        ...
                        else if (typeName.IndexOf("GameEvents" + GM.missionPath + ".") == 0)
                        {
                            shortName = typeName.Substring(string.Concat("GameEvents.", GM.missionPath).Length);
                            Debug.Log(shortName);
                            assets[shortName] = t;
                        }
                    }
