...
public class GM
{
    ...
    Dictionary<string, UIeX> UIObjects;
    ...
    public static void NewGame()
    {
        ...
        GM.instance.UIObjects = new Dictionary<string, UIeX>();
    ...
    public static void InitGame(JSONNode node)
    {
    ...
    public static SkyObject GetObject(string ID)
    {
        ...
        else if (GM.instance.UIObjects.ContainsKey(ID))
            return GM.instance.UIObjects[ID];
        else
    ...
    public static void AddUIObject(UIeX obj)
    {
        if(GM.instance.UIObjects.ContainsKey(obj.ID))
        {
            Debug.LogError("Already has UIObject: " + obj.ID);
            return;
        }

        GM.instance.UIObjects.Add(obj.ID, obj);
    }
    ...
    void InitUniqs()
    {
        ...
        icons = new List<SceneObject>(Resources.LoadAll<SceneObject>("missions/" + GM.missionPath + "/Prefabs"));
        foreach (var sprite in icons)
        {
            string key = sprite.name;
            SceneObject temp = GameObject.Instantiate<SceneObject>(sprite);
